...
BUILDINGS = [
    {
        "building_id": "BLD-001", "name": "绿地办公大厦", "building_type": "office",
    ...
    {
        "building_id": "BLD-002", "name": "中央商业广场", "building_type": "commercial",
    ...
    {
        "building_id": "BLD-003", "name": "滨河花园小区", "building_type": "residential",
    ...
    {
        "building_id": "BLD-004", "name": "临朐县人民医院", "building_type": "hospital",
    ...
    {
        "building_id": "BLD-005", "name": "临沂大学教学楼A", "building_type": "school",
    ...
    for bld in BUILDINGS:
        ...
        while ts <= end_hour:
            ...
            weather_records.append({
                "building_id": bld["building_id"], "timestamp": ts.isoformat(),
                ...
                for ch in chillers:
                    ...
                    if random.random() < anomaly_rate * 0.5:
                        ...
                        anomaly_events.append({
                            "building_id": bld["building_id"], "device_id": ch["device_id"],
            ...
            if hr_f > 0.2:
                for ahu in ahus:
                    ...
                    if random.random() < anomaly_rate * 0.3 and cooling:
                        ...
                        anomaly_events.append({
                            "building_id": bld["building_id"], "device_id": ahu["device_id"],
            ...
            if hr_f > 0.3:
                for vav in vavs:
                    ...
                    if random.random() < anomaly_rate * 0.3 and cooling:
                        ...
                        anomaly_events.append({
                            "building_id": bld["building_id"], "device_id": vav["device_id"],
...
def seed_database(data: dict, reset: bool = False):
    ...
    try:
        ...
        if count > 0 and not reset:
            print(f"Database already has {count} buildings. Use --reset to clear first.")
            return
        ...
        for b in data["buildings"]:
            cur.execute(
                ...
                (id, building_id, name, building_type, area, address, floors, year_built,
                 climate_zone, cooling_area, design_cooling_load, design_heating_load)
                VALUES (%(id)s, %(building_id)s, %(name)s, %(building_type)s, %(area)s,
        ...
        for e in data["equipment"]:
            cur.execute(
                ...
                (id, building_id, device_id, device_name, device_type, system_type,
                 model, manufacturer, rated_power_kw, rated_capacity, rated_cop,
                 location, install_date, status)
                VALUES (%(id)s, %(building_id)s, %(device_id)s, %(device_name)s, %(device_type)s,
        ...
        _batch_insert(cur,
            ...
            (building_id, timestamp, dry_bulb_temp, wet_bulb_temp, relative_humidity,
             wind_speed, solar_radiation, atmospheric_pressure)
            VALUES (%(building_id)s, %(timestamp)s, %(dry_bulb_temp)s, %(wet_bulb_temp)s,
        ...
        _batch_insert(cur,
            ...
            VALUES (%(building_id)s, %(timestamp)s, %(total_electricity_kwh)s,
        ...
        for ae in data["anomaly_events"]:
            cur.execute(
                ...
                (id, building_id, device_id, timestamp, anomaly_type, severity,
                ...
                VALUES (%(id)s, %(building_id)s, %(device_id)s, %(timestamp)s,
